...
async def my_profile_endpoint(current_user_id: Annotated[int, Depends(verify_token)], _=Depends(process_user_last_online_update)):
...
@users_router.get("/avatar/{user_id}", status_code=status.HTTP_200_OK)
