...
class FileManager:
    ...
    def init_folders(self):
        ...
        for folder in [avatars_folder, messages_folder]:
        ...
        with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zip_file:
            for avatar_path in paths:
                if self.file_exists(avatar_path):
                    zip_file.write(avatar_path, arcname=avatar_path.name)  # Добавляем файл в архив

        # Перемещаем курсор в начало буфера для чтения
        zip_buffer.seek(0)
    ...
    def validate_image(file: UploadFile) -> None:
        ...
        file_size_mb = len(file.file.read()) / (1024 * 1024)  # Размер в мегабайтах
        file.file.seek(0)  # Сбросить указатель чтения для последующей работы
        ...
        try:
            ...
            image.verify()  # Проверяет валидность изображения
            file.file.seek(0)  # Сбросить указатель чтения
        except Exception:
